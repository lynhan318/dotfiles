...
#
# Workspace 5 is special: it's the floating scratch space on the secondary
# screen (the Yoga panel), so the window is floated and centered there instead
# of being tiled. 1-4 land on the main screen (the Samsung).
...
if [ "$WORKSPACE" = "$FLOAT_WORKSPACE" ]; then
