  ...
  implements IDebugAdapter
{
  ...
  // PATCH(nvim-dap race): resolves once `Debugger.enable` has settled, which is
  // also when JSC has finished replaying `Debugger.scriptParsed` for scripts
  // that were already loaded (i.e. every script, when attaching to a running
  // process). Used to gate the `initialized` event so breakpoints aren't set
  // before their sources are known. See `["Inspector.connected"]`.
  #debuggerReady?: Promise<unknown>;
  ...
  public initialize(request: InitializeRequest): DAP.InitializeResponse {
    ...
    if (request.enableDebugger !== false) {
      // PATCH(nvim-dap race): keep the enable promise so `["Inspector.connected"]`
      // can wait for the `scriptParsed` replay before announcing `initialized`.
      this.#debuggerReady = this.send("Debugger.enable").catch(error => {
  ...
  ["Inspector.connected"](): void {
    ...
    // PATCH(nvim-dap race): don't announce `initialized` on socket open. Clients
    // like nvim-dap send `setBreakpoints` the instant they see `initialized`; if
    // that happens before `Debugger.enable` has replayed `scriptParsed` for the
    // already-loaded scripts, `#setBreakpointsByUrl` can't find the source and
    // files a line-0 placeholder that waits for a `breakpointResolved` event that
    // never fires (the script already resolved). Result: breakpoints in a running
    // process never bind. Waiting for enable to settle first makes those sources
    // known before the client sets breakpoints. Falls back to immediate emit if
    // the debugger wasn't enabled or enable rejected, so the session never hangs.
    const ready = this.#debuggerReady ?? Promise.resolve();
    ready.then(
